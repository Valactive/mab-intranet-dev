			...
			if($rules['handle'] == $section)
			{
				...
				if($rules['own_entries'] == 1 || $rules['edit'] == 0 || $rules['delete'] == 0 || $rules['use_filter'] == 1)
				{
					...
					if($rules['own_entries'] == 1)
					{
						...
						$current_section = Symphony::Database()->fetch('SELECT `id` FROM `tbl_sections` WHERE `handle` =\'' . $rules['handle'] .'\';');
						//ok, now, we find entries for this author in current section
						$id_sym_field = Symphony::Database()->fetch('SELECT `id` FROM `tbl_fields` WHERE parent_section = '. $current_section[0]['id'].' AND type = \'author\'');
						// Only get the ID's of the current author to begin with:
						$results = Symphony::Database()->fetch('SELECT `entry_id` as `id` FROM `tbl_entries_data_'.$id_sym_field[0]['id'].'` WHERE `author_id` = '.$id_author.';');
