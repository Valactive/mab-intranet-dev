...
(function($) {
	...
	$(document).ready(function() {
		...
		// Custom css by vf 20 juin 2013
		// colorise les td ayant le texte "Fait" en vert et "A faire" en rouge
		$('td.field-182').each(function(i) {
			if($(this).html() == 'Fait'){
				$(this).empty().append('<span class="btn green">Fait</span>');
			} else if ($(this).html() == 'A faire' ) {
				$(this).empty().append('<span class="btn red">A faire</span>');
			} else {
				$(this).empty().append('<span class="btn orange">Non traité</span>');
			}
		});
	});
})(jQuery.noConflict());
